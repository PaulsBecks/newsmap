...
ZEIT_ENRICHED_DATA_PATH = "zeit_data_enriched.json"
class ZeitCrawler(Crawler):
    
    def __init__(self):
        super().__init__()
        self.url = "https://newsfeed.zeit.de/news/index"
        self.source = "ZEIT"
        with open(ZEIT_ENRICHED_DATA_PATH) as f:
    ...
    def persist_news(self, news):
        ...
        with open(ZEIT_ENRICHED_DATA_PATH, "w") as f:
            json.dump(self.enriched_news, f, cls=self.encoder)
        print("Data enriched and saved to zeit_data_enriched.json")