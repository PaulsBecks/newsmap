...
MDR_ENRICHED_DATA_PATH = "mdr_data_enriched.json"
class MdrCrawler(Crawler):
    
    def __init__(self):
        super().__init__()
        self.url = "https://www.mdr.de/nachrichten/index-rss.xml"
        self.source = "MDR"
        with open(MDR_ENRICHED_DATA_PATH) as f:
    ...
    def persist_news(self, news):
        ...
        with open(MDR_ENRICHED_DATA_PATH, "w") as f:
            json.dump(self.enriched_news, f, cls=self.encoder)
        print("Data enriched and saved to mdr_data_enriched.json")