...
SPIEGEL_ENRICHED_DATA_PATH = "spiegel_data_enriched.json"
class SpiegelCrawler(Crawler):
    
    def __init__(self):
        super().__init__()
        self.url = "https://www.spiegel.de/schlagzeilen/tops/index.rss"
        self.source = "SPIEGEL"
        with open(SPIEGEL_ENRICHED_DATA_PATH) as f:
            self.enriched_news = json.load(f)
            
    def crawl_news(self) -> List[NewsDto]:
        print("Crawling SPIEGEL")
    ...
    def persist_news(self, news):
        ...
        with open(SPIEGEL_ENRICHED_DATA_PATH, "w") as f:
            json.dump(self.enriched_news, f, cls=self.encoder)
        print("Data enriched and saved to spiegel_data_enriched.json")